-- Complete BIPA 1 Data Insert Script
-- This script matches the actual database schema based on entity structures
...
-- Step 3: Insert Unit Levels using unit IDs
unit_levels_insert AS (
  ...
  FROM units_insert u
  JOIN (VALUES 
    (1, 'Unit 1', 'Unit perkenalan dengan sapaan dan memperkenalkan diri', 1),
    (2, 'Unit 2', 'Unit transportasi umum dan komunikasi dengan driver', 2),
    (3, 'Unit 3', 'Level mencari kos, hotel, dan asrama', 3),
...
content_items_insert AS (
  ...
  ) AS content_data(level_position, content_type, title, description, content, position, metadata) ON ul.position = content_data.level_position
...
quizzes_insert AS (
  ...
  ) AS quiz_data(level_position, content_position, question, explanation) 
  ON ul.position = quiz_data.level_position AND ci.position = quiz_data.content_position
...
quiz_options_insert AS (
  ...
  ) AS option_data(level_position, content_position, option_text, is_correct)
  ON ul.position = option_data.level_position AND ci.position = option_data.content_position
...
roleplays_insert AS (
  ...
  ) AS roleplay_data(level_position, content_position, scenario, instructions, character_name, character_description)
  ON ul.position = roleplay_data.level_position AND ci.position = roleplay_data.content_position
...
) AS turn_data(level_position, content_position, speaker, message, turn_order)
ON ul.position = turn_data.level_position AND ci.position = turn_data.content_position;
...
-- ✅ 5 Unit Levels: One for each unit
-- ✅ 19 Content Items: 15 quizzes + 4 roleplays (video content skipped as requested)
